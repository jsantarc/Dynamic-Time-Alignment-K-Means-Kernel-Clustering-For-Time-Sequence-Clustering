...
function [startstop,X,y] = LinearToy();
...
while TotalSize<1000    
    ...
    X(START(n):STOP(n),1)=[0:NumberSamples]-NumberSamples/2+4*randn(1,1);
    X(START(n):STOP(n),2)=X(START(n):STOP(n),1)+10*randn(1,1);
...
while TotalSize<2000
    ...
    %load  array with toy data generated using a differnt linear function  
    X(START(n):STOP(n),1)=[0:NumberSamples]-NumberSamples/2+4*randn(1,1);
    X(START(n):STOP(n),2)=-(X(START(n):STOP(n),1)+10*randn(1,1));
...
title('Toy Data:True Linear Centroids' )
